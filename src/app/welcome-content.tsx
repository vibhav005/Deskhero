...
import { ArrowRight, LogIn, Droplet, Footprints, Timer, Shield } from "lucide-react";
...
export function WelcomeContent() {
  return (
    ...
    >
      <div aria-hidden className="pointer-events-none absolute -right-24 -top-24 h-64 w-64 rounded-full bg-primary-soft blur-2xl" />
      <div aria-hidden className="pointer-events-none absolute -bottom-24 -left-20 h-56 w-56 rounded-full bg-accent-soft blur-2xl" />
      ...
      <div className="relative flex flex-1 flex-col justify-center py-10">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }}>
          ...
          <p className="mt-4 text-pretty text-base leading-relaxed text-muted-foreground">
            DeskHero turns tiny bursts of movement, hydration, and posture care into a friendly
            game. No gym, no equipment, and no pressure — just small wins that fit around a busy
            workday. Completely free, always.
          </p>
        </motion.div>
      </div>
